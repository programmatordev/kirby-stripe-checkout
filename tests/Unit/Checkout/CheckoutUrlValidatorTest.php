...
final class CheckoutUrlValidatorTest extends TestCase
{
    ...
    public function testDestinationPolicy(string $url, bool $requiresHttps, bool $valid): void
    {
        $this->assertSame($valid, CheckoutUrlValidator::isDestination($url, $requiresHttps));
    }

    /** @return iterable<string, array{string, bool, bool}> */
    public static function destinations(): iterable
    {
        yield 'HTTP allowed' => ['http://store.example/checkout', false, true];
        yield 'HTTPS required and used' => ['https://store.example/checkout', true, true];
        yield 'local HTTP exception' => ['http://store.ddev.site/checkout', true, true];
        yield 'public HTTP rejected' => ['http://store.example/checkout', true, false];
    ...
    {
        $this->assertTrue(CheckoutUrlValidator::isPersistedDestination('http://store.example/checkout', requiresHttps: false));
