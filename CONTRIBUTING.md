...
The home page and Products page show the catalogue. Product pages have option selectors, a live selected price and add-to-cart forms. The shared storefront keeps one cart on every page: a sticky sidebar on desktop and a collapsible panel above the content on smaller screens. Its update/remove/clear controls use the real plugin routes and the project-owned HTML renderer in `site/config/config.php`. These templates, snippets and their small JavaScript examples are excluded from the Composer package. The Test pages menu keeps hosted/embedded and return-page previews available without pretending to submit Checkout or create orders.

Use the EN/PT links to switch the current product's language. Product content, options, amounts and plugin errors follow the active language; the development storefront's own interface copy is intentionally English. Product details and a Panel edit link are tucked into a disclosure on each product. Missing product images have an explicit placeholder; configured Kirby images are displayed on cards, the product page and cart items.

For offline manual checks, use Kirby as the price source with EUR configured and the committed physical/digital/option fixtures. Test adding twice, changing quantity, selecting another variant, removing and clearing. The option product includes an unavailable Digital/English combination; it must disable Add to cart. Different option combinations must appear as separate cart lines. Open two tabs and confirm that a stale update reports a conflict with the current cart rather than overwriting it. The Refresh cart control reads the latest state without replaying a write. Controls are temporarily locked while a request is pending. Check both desktop and phone widths, including a long cart's scroll area. Switch to `/pt/cart` to verify the same selections and translated errors. An incognito browser should have an independent cart.
