...
`TaxCode::requiresPerformanceLocation()` returns `true` or `false` when requirement metadata is attached, or `null` when it is unknown. Product resolution checks catalogue membership but keeps ID-only values; it does not attach these extra facts or inspect the Session request filter.
