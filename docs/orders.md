...
A selected shipping snapshot must come from an expanded Stripe Shipping Rate. A scalar Rate ID identifies the object but cannot freeze its label, amount, estimate or tax classification. Reconciliation therefore needs to retrieve or expand that object before normalization. Digital Checkout results retain an explicit zero `shippingTotal` without inventing a selected Rate or shipping snapshot. The selected Rate's fixed amount must agree with Stripe's returned shipping subtotal, and the snapshot total must agree with `shippingTotal`; the plugin does not derive the final total locally because Stripe may apply tax and discounts.
